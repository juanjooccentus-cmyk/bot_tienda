...
APP_SECRET   = os.getenv("WABA_APP_SECRET", "")  # opcional (firma)
...
def is_price_query(text: str) -> bool:
    t = text.lower()
    return any(k in t for k in ["precio", "vale", "cuánto", "coste", "cuesta", "€", "eur"])
...
def verify_signature(req):
    """Opcional: valida X-Hub-Signature-256 si configuras APP_SECRET."""
    if not APP_SECRET: 
        return True
                ...
                for msg in value.get("messages", []):
                    ...
                    if msg.get("type") == "text":
                        ...
                        if is_price_query(body):
                            ...
                            send_whatsapp(from_, "Pídeme un precio: ‘precio mochila conejo jellycat’")
        return "ok", 200
    except Exception:
        return "ok", 200
